...
def process(pc_base_directory):
    logger.info("Processing script is executing.")
    ...
    n_splits = 3
    train_validation_split = StratifiedShuffleSplit(
        n_splits=n_splits, test_size=0.2, random_state=41
    )
    ...
    for i, (train_index, valid_index) in enumerate(
        train_validation_split.split(X_train_full, y_train_full)
    ):
        X_train, X_valid = (
            X_train_full.iloc[train_index],
            X_train_full.iloc[valid_index],
        )
        y_train, y_valid = (
            y_train_full.iloc[train_index],
            y_train_full.iloc[valid_index],
        )
    ...
    for id, t_data, v_data in zip(range(n_splits), train_data, validation_data):
        ...
        t_validation_data.append(
            pd.DataFrame(np.concatenate([tX_validation, ty_validation], axis=1))
        )
...
def _save_baseline(pc_base_directory, train_data, test_data, validation_data):
    ...
    for i, data in enumerate(train_data):
        ...
        data.to_csv(path / f"train-baseline-{i+1}.csv", header=False, index=False)
...
def _save_transformers(pc_base_directory, transformers_list):
    ...
    with tempfile.TemporaryDirectory() as directory:
        for i, transformers in zip(range(len(transformers_list)), transformers_list):
            joblib.dump(
                transformers, os.path.join(directory, f"transformers_{i+1}.joblib")
            )

        with tarfile.open(
            transformers_path / "transformers.tar.gz", "w:gz"
        ) as tar_file:
